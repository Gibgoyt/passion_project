...
static int test_allocation_performance(void) {
    ...
    const int num_tests = 5; // Reduced to avoid running out of pages
    const size_t test_sizes[] = {512, 1024, 2048}; // All within MAC_JWT_MAX_LENGTH (2048)
    ...
    for (int size_idx = 0; size_idx < num_sizes; size_idx++) {
        ...
        for (int i = 0; i < num_tests; i++) {
            ...
            if (!buffer) {
                printf("❌ Allocation failed for size %zu on iteration %d\n", test_size, i);
                return -1;
            }
